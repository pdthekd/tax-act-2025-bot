...
# --- PAGE CONFIGURATION ---
st.set_page_config(page_title="Tax Act 2025 Assistant", page_icon="⚖️")
...
# --- FILE CONFIGURATION (Trying to load ALL files) ---
@st.cache_resource
def upload_knowledge_base():
    # We uncomment ALL files to try and load the full database
    file_names = [
    ...
    uploaded_files = []
    status_bar = st.progress(0, text="Attempting to load all files...")
    ...
    for i, filename in enumerate(file_names):
        try:
            ...
            # Wait for Google to process the PDF
...
# --- MAIN APP ---
st.title("⚖️ Tax Act 2025 Bot (Full Version)")
...
if prompt := st.chat_input("Ask about any section..."):
    st.chat_message("user").write(prompt)
    st.session_state.messages.append({"role": "user", "content": prompt})

    try:
        # --- THE TRICK IS HERE ---
        # We manually type 'gemini-1.5-flash' even if it's not in your menu.
        # This model is usually available to EVERYONE and handles large files better.
        ...
        st.error(f"Error: {e}")
        st.info("💡 If this failed, your API key is strictly limited. Please revert to the 'Diet' version (only Act + Mapping).")
